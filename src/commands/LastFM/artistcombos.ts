...
export default class ArtistCombos extends LastFMCommand {
	...
	async run(args:string) {
		...
		Combos AS (
			SELECT headnr, artist, MIN(timeDiff) AS "minDiff", (IFNULL(LEAD(rownr) OVER (ORDER BY headnr), (SELECT COUNT(*) FROM scrobbles WHERE lastfmsession=?)) - rownr) AS "combo" FROM Heads GROUP BY headnr, artist
		)
		...
		embed.setTitle(`${this.sanitizeMarkdown(safe[0])}'s top artist combos`);
